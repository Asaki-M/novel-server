 ...
 */
export const validateAndProcessChatRequest = async (req: Request): Promise<ProcessedChatRequest> => {
  const {
    messages,
    temperature,
    max_tokens,
 ...
 */
export const processCharacter = async (characterId: string): Promise<string | undefined> => {
  if (!characterId) {
    return undefined;
  }
 ...
 * @throws Error - 验证失败时抛出错误
 */
export const validateCharacterId = (characterId: string): void => {
  if (!characterId || typeof characterId !== 'string' || characterId.trim().length === 0) {
