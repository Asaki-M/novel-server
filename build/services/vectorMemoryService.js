...
class VectorMemoryService {
    ...
    openai;
    sessions = new Map();
    pendingMessages = new Map(); // 等待总结的消息
    constructor() {
     ...
     * 添加消息并智能处理
     */
    async addMessage(sessionId, message, chunkThreshold = 8) {
     ...
     * 检索相关记忆
     */
    async retrieveRelevantMemory(sessionId, query, retrievalCount = 5) {
        const session = this.sessions.get(sessionId);
        if (!session)
            throw new Error('Session not found');
        // 1. 生成查询向量
     ...
     * 分析消息内容
     */
    async analyzeMessage(messages, session) {
...
`;
     ...
     * 创建记忆块
     */
    async createMemoryChunk(sessionId, messages, analysis) {
        const session = this.sessions.get(sessionId);
        // 生成内容和总结
     ...
     * 创建初始记忆块
     */
    async createInitialChunk(sessionId, systemMessage) {
        const session = this.sessions.get(sessionId);
     ...
     * 生成总结
     */
    async generateSummary(content, session) {
        const prompt = `
...
`;
     ...
     * 获取最近的记忆块
     */
    async getRecentChunks(sessionId, count) {
     ...
     * 生成剧情总结
     */
    async generatePlotSummary(relevantChunks, recentChunks) {
...
`;
     ...
     * 提取活跃角色
     */
    extractActiveCharacters(relevantChunks, recentChunks) {
